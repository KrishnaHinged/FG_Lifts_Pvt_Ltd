...
export default function EditMilestoneClient({ milestone }) {
  ...
  const handleSubmit = async (payload) => {
    ...
    setErrorMsg('')
    try {
      const res = await fetch(`/api/admin/timeline/${milestone.id || milestone._id}`, {
        method: 'PUT',
      ...
      if (res.ok && data.success) {
        ...
        setErrorMsg(data.error || 'Failed to update milestone.')
      }
    } catch {
    ...
    <div className="space-y-6 select-none max-w-4xl mx-auto">
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-sans font-bold text-gray-900 text-2xl tracking-tight leading-none m-0">
          Edit Milestone: {milestone.year} - {milestone.title}
        </h1>
      </div>
      ...
      <TimelineMilestoneForm milestone={milestone} onSubmit={handleSubmit} isLoading={isLoading} />
    </div>
  )
}
